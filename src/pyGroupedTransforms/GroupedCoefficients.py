...
class GC:   # Superclass of GroupedCoefficientsComplex and GroupedCoefficientsReal. 
    ...
    def __getitem__(self, idx): 
        ...
        if type(idx) == int:
            ...
            raise TypeError("Index must be an integer or a tuple that contains integer or the empty tuple")
    ...
    def __setitem__(self, idx, Number):
        ...
        if type(idx) == int:
            ...
            if isinstance(self, GroupedCoefficientsComplex) and np.isrealobj(Number) or isinstance(self, GroupedCoefficientsReal) and np.iscomplexobj(Number):
                raise TypeError("Type mismatch.")
    ...
    def set_data(self, data):
        ...
        """
        if isinstance(self, GroupedCoefficientsComplex) and np.isrealobj(data) or isinstance(self, GroupedCoefficientsReal) and np.iscomplexobj(data):
                raise TypeError("Type mismatch.")
        else: 
            self.data[:] = data
            return None
    ...
    def norms(self, Dict=False, other = None, m = None):
        ...
        if other == None:
            if m == None:
                if Dict == False:
                    return [np.linalg.norm(self[self.settings[i].u])
                ...
                if Dict == False:
                    if self.settings[0].mode == CWWTtools:   #does not work (in julia and in python... ?)
                        ...
                        for i in range(len(self.settings)):   
                            ...
                            for jj in range(freq.shape[1]):
                                ...
                                if d > 1:
                                    for dd in range(1,d):
                                        Psi = np.kron(Psi, circulant(variances(j[dd],m)))
                    ...
                    if self.settings[0].mode == CWWTtools:   #doesn't work... ?
                        ...
                        for i in range(len(self.settings)):
                            if len(self.settings[i].u) != 0:
                                dd[tuple(self.settings[i].u)] = np.linalg.norm(self[self.settings[i].u])
                        return dd

        else: #other != None
            c = GroupedCoefficients(self.settings, (np.sqrt(np.real(other.data))) * self.data)
...
def variances(j,m):   
    ...
    if m == 2:
        ...
        if j == 0:
            ...
            y = [0.098443287037094, 0.024151620370357, -0.006407407407473,
                 -2.997685185108751e-04, 1.157407410282006e-06,
                 -2.997685185108751e-04, -0.006407407407473, 0.024151620370]
        else:
            y = np.zeros(2**j)
            y[0:5] = [0.098443287037052, 0.024151620370458, -0.006407407407375,
                      -2.997685186147822e-04, 5.787036288131668e-07]
            y[-5:-1] = y[1:5]
        ...
        if j == 0:
            ...
            y = [0.041543521817902, 0.010977080308839,
                 -0.009529428467484, 0.010977080308902]
        elif j == 3:
            y = [0.041534149423629, 0.011629855618373,
                 -0.004764714233735, -6.527753094869201e-04,
                 9.372394228547530e-06, -6.527753094788768e-04,
                 -0.004764714233719, 0.011629855618379]
        else:
            y = np.zeros(2**j)
            y[0:7] = [0.041534149423653, 0.011629855618368, -0.004764714225921,
                      -6.528682451447852e-04, 4.686197131487609e-06,
                      9.293564145017154e-08, -7.821712475559538e-12]
            y[-7:-1] = y[1:7]
    ...
    def __eq__(self, other):
        if isinstance(other, Setting):
            ...
            if len(self.bandwidths) == 0 == len(other.bandwidths):
                boollist[2] = True
            elif len(self.bandwidths) == len(other.bandwidths) and (self.bandwidths == other.bandwidths).all():
                boollist[2] = True  
