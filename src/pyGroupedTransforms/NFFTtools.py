...
def index_set_without_zeros(bandwidths: np.array) -> np.array:    #nfft_index_set_without_zeros(bandwidths::Vector{Int})::Array{Int}
    """
    ...
    if d == 0: return np.array([0])
    if d == 1: return np.array([list(range(-int(bw//2),0)) + list(range(1, int(bw//2))) for bw in bandwidths])

    tmp = tuple([list(range(-int(bw//2),0)) + list(range(1, int(bw//2))) for bw in bandwidths])
...
def nfft_index_set(bandwidths: np.array) -> np.array:    #nfft_index_set(bandwidths::Vector{Int})::Array{Int}
    ...
    d = len(bandwidths)
    if d == 0: return np.array([0],dtype = 'int')
    if d ==1: return np.array(list(range(int(-bandwidths[0]/2), int(bandwidths[0]/2))), dtype = 'int')
...
def nfft_mask(bandwidths: np.array) -> np.array:   #nfft_mask(bandwidths::Vector{Int})::BitArray{1}
    """
...
def get_transform(bandwidths: np.array, X: np.array):   #get_transform(bandwidths::Vector{Int}, X::Array{Float64})::LinearMap
    """
    ...
    if len(bandwidths) == 0:
        return DeferredLinearOperator(dtype=np.complex128, shape=(M, 1), mfunc= lambda fhat: np.full(M, fhat[0]), rmfunc=lambda f: np.array([np.sum(f)]))
    ...
    return DeferredLinearOperator(dtype=np.complex128, shape=(M, N), mfunc=trafo, rmfunc=adjoint)  



def get_matrix(bandwidths, X):   #get_matrix(bandwidths::Vector{Int}, X::Array{Float64})::Array{ComplexF64}
    """
    ...
    if d == 1:
        ...
        F_direct = np.array([[np.exp(-2j * np.pi * np.dot(x, n)) for n in freq.T] for x in X.T])
