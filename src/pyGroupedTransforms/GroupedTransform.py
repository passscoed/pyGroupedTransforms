...
systems = {"exp" : NFFTtools, "cos" : NFCTtools, "chui1" : CWWTtools, "chui2": CWWTtools, "chui3": CWWTtools, "chui4": CWWTtools, "mixed" : NFMTtools}
...
def get_setting(system, N, U = None, d = None, ds = None):      #I have to add bases and NFMT stuff here!
    ...
    if d != None:           # Input system::String, d::Int,  ds::Int, N::Vector{Int}
        ...
        if systems[system] == NFMTtools:
            if len(basis_vect) == 0:
                ValueError("please call get_setting with basis_vect for a NFMT transform.")
            if len(basis_vect) < d:
                ValueError("basis_vect must have an entry for every dimension.")
            return [ Setting(u=U[idx], mode=systems[system], bandwidths=np.array(bwl[idx], 'int32'), bases = basis_vect[U[idk]]) for idx in range(len(U)) ]
            
        else:
            return [ Setting(u=U[idx], mode=systems[system], bandwidths=np.array(bwl[idx], 'int32'))   for idx in range(len(U)) ]
    ...
    if type(N[0]) == int:     # Input system::String,  U::list{list{Int}},  N::list{Int}
        ...
        if systems[system] == NFMTtools:
            ...
            if len(basis_vect) < max(max(u) for u in U):
                ValueError("basis_vect must have an entry for every dimension.")
            return [ Setting(u=u, mode=systems[system], bandwidths=np.array(bwl[i], 'int32'), bases = basis_vect[u]) for i,u in enumerate(U) ]
            
        else:
            return [Setting(u=u, mode=systems[system], bandwidths=np.array(bwl[i], 'int32')) for i,u in enumerate(U)]


    elif type(N[0]) == list or type(N[0]) == np.ndarray:   # Input system::String,  U::list{list{Int}},  N::list{list{Int}} oder list{np.ndarray}
        ...
        for i, u in enumerate(U):
            if len(u) == 0:
                ...
                if len(N[i]) != len(u):
                    raise ValueError("Vector N has for the set", u, "not the right length")
                bwl[i] = N[i]
...
class GroupedTransform:
    def __init__(self, system, X, settings = [], fastmult=True, basis_vect=[], N = [], U = None, d = None, ds = None):  
        ...
        if system == "mixed":
            if len(basis_vect) == 0:
                ValueError("please call GroupedTransform with basis_vect for a NFMT transform.")
        ...
        if system in {"exp", "chui1", "chui2", "chui3", "chui4"}:
            ...
            if np.sum(cosine_mask) > 0:
                if (np.min(X[cosine_mask, :]) < 0) or (np.max(X[cosine_mask, :]) > 1):
                    raise ValueError("Nodes must be between 0 and 1 for cosine or Chebyshev dimensions.")

            exp_mask = ~cosine_mask
            if np.sum(exp_mask) > 0:
                if (np.min(X[exp_mask, :]) < -0.5) or (np.max(X[exp_mask, :]) > 0.5):
                    raise ValueError("Nodes must be between -0.5 and 0.5 for exponentional dimensions.")
        ...
        if fastmult:    
            ...
            self.transforms = [DeferredLinearOperator() for i in range(len(self.settings))]
            for idx, s in enumerate(self.settings):
                ...
                if system.startswith("chui"):    
                    Order = int(system[-1])
                    self.transforms[idx] = s.mode.get_transform(bandwidths = s.bandwidths, X = np.copy(X[:, u], order = "C"), m = Order) 
                elif system == "mixed":
                    self.transforms[idx] = s.mode.get_transform(bandwidths = s.bandwidths, X = np.copy(X[:, u], order = "C"), bases = s.bases)   
                else:
                    self.transforms[idx] = s.mode.get_transform(bandwidths = s.bandwidths, X = np.copy(X[:, u], order = "C"))          
        else:        
            self.transforms = []
            s1 = self.settings[0]
            if len(s1.bandwidths) == 0:
                ...
                u1 = s1.u
            if system.startswith("chui"):
                raise ValueError("Direct computation with full matrix not supported for wavelet basis.")
            elif system == "mixed":
                matrix = np.array(s1.mode.get_matrix(bandwidths = s1.bandwidths, X = X[:, u1].T), bases = s1.bases)    
                for s in self.settings[1:]:
                    if len(s.bandwidths) == 0:
                        u = (0,)  
                    else:
                        u = s.u
                    matrix = np.hstack([matrix, np.array(s.mode.get_matrix(s.bandwidths, X[:, u].T), bases = s.bases)])    
            else:
                matrix = np.array(s1.mode.get_matrix(bandwidths = s1.bandwidths, X = X[:, u1].T))    
                for s in self.settings[1:]:
                    if len(s.bandwidths) == 0:
                        u = (0,)  
                    else:
                        u = s.u
                    matrix = np.hstack([matrix, np.array(s.mode.get_matrix(s.bandwidths, X[:, u].T))])    
            self.matrix = matrix
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, np.ndarray):  # `f = F*f`    (f = other)
            if self.fastmult:
                ...
                return GroupedCoefficients(self.settings, (self.matrix.conj()).T @ other)  
        elif isinstance(other, GC):  #`f = F*fhat`    (fhat = other)
            if self.settings != other.settings:   
                raise ValueError("The GroupedTransform and the GroupedCoefficients have different settings")
    ...
    def __getitem__(self, u):
        ...
        """
        idx = next((i for i, s in enumerate(self.settings) if len(s.u) == len(u) and s.u == u), None)
