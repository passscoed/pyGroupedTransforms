...
def index_set_without_zeros(bandwidths: np.array) -> np.array:    #nfct_index_set_without_zeros(bandwidths::Vector{Int})::Array{Int}
    ...
    if d == 1: return np.array([1] + list(range(2, bandwidths[0])))
...
def nfct_index_set(bandwidths: np.array) -> np.array:    #nfct_index_set(bandwidths::Vector{Int})::Array{Int}
    ...
    if d ==1: return np.array([i for i in range(0,bandwidths[0])], 'int')
...
def nfct_mask(bandwidths: np.array) -> np.array:   #nfft_mask(bandwidths::Vector{Int})::BitArray{1}
    """
...
def get_transform(bandwidths: np.array, X: np.array):   #get_transform(bandwidths::Vector{Int}, X::Array{Float64})::LinearMap
    ...
    if len(bandwidths) == 0:
        return DeferredLinearOperator(dtype=np.float64, shape=(M, 1), mfunc= lambda fhat: np.full(M, fhat[0]), rmfunc=lambda f: np.array([np.sum(f)]))
    ...
    return DeferredLinearOperator(dtype=np.float64, shape=(M, N), mfunc=trafo, rmfunc=adjoint)  
...
def get_matrix(bandwidths, X):   #function get_matrix(bandwidths::Vector{Int}, X::Array{Float64})::Array{Float64}
    """
    ...
    if d == 1:   
        freq = freq.flatten()
        F_direct = np.array([get_multiplier(n) * np.cos(2 * np.pi * x * n)for x in X for n in freq]).reshape(M, -1)
    else:
        F_direct = np.array([get_multiplier(n) * np.prod(np.cos(2 * np.pi * x * n)) for x in X.T for n in freq.T]).reshape(M, -1)
